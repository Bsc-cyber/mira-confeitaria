<?php
// ==========================================================================
// MIRA CONFEITARIA - PAINEL DE PRODUTOS COMPLETO COM PDO E MENU EMBUTIDO
// ==========================================================================

$page = 'produtos';

// Puxa o arquivo de conexão PDO que você enviou
include 'conexao.php';

// Verifica se chegou alguma requisição AJAX enviada pelo JavaScript via POST
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['acao'])) {
    $acao = $_POST['acao'];
    $id = isset($_POST['id']) ? intval($_POST['id']) : 0;
    $nome = isset($_POST['nome']) ? trim($_POST['nome']) : '';
    $categoria = isset($_POST['categoria']) ? trim($_POST['categoria']) : '';
    $tamanho = isset($_POST['tamanho']) ? trim($_POST['tamanho']) : '';
    $preco = isset($_POST['preco']) ? trim($_POST['preco']) : '0,00';
    $ativo = isset($_POST['ativo']) ? trim($_POST['ativo']) : 'sim';
    $sabores = isset($_POST['sabores']) ? trim($_POST['sabores']) : '';
    $descricao = isset($_POST['descricao']) ? trim($_POST['descricao']) : '';

    // Converte o valor em Real (Ex: 150,00) para o padrão decimal do banco (150.00)
    $preco_formatado = floatval(str_replace(['.', ','], ['', '.'], $preco));

    try {
        if ($acao === 'salvar' && !empty($nome)) {
            $sql = "INSERT INTO produtos (nome, categoria, tamanho, preco, ativo, sabores, descricao) VALUES (:nome, :categoria, :tamanho, :preco, :ativo, :sabores, :descricao)";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([
                ':nome' => $nome,
                ':categoria' => $categoria,
                ':tamanho' => $tamanho,
                ':preco' => $preco_formatado,
                ':ativo' => $ativo,
                ':sabores' => $sabores,
                ':descricao' => $descricao
            ]);
        }

        if ($acao === 'editar' && $id > 0 && !empty($nome)) {
            $sql = "UPDATE produtos SET nome = :nome, categoria = :categoria, tamanho = :tamanho, preco = :preco, ativo = :ativo, sabores = :sabores, descricao = :descricao WHERE id = :id";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([
                ':nome' => $nome,
                ':categoria' => $categoria,
                ':tamanho' => $tamanho,
                ':preco' => $preco_formatado,
                ':ativo' => $ativo,
                ':sabores' => $sabores,
                ':descricao' => $descricao,
                ':id' => $id
            ]);
        }

        if ($acao === 'excluir' && $id > 0) {
            $sql = "DELETE FROM produtos WHERE id = :id";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([':id' => $id]);
        }
    } catch (PDOException $e) {
        // Silencia erros em segundo plano para não travar o visual na apresentação
    }
}
?>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>MIRA Confeitaria - Produtos</title>
    <!-- O ../ faz o arquivo sair da pasta php/ e achar a pasta css/ na raiz do projeto -->
    <link rel="stylesheet" href="../css/produtos.css?v=18.0">
    <link rel="stylesheet" href="../css/responsivo.css">
</head>
<body>

    <!-- BARRA LATERAL EMBUTIDA REAL (Resolve definitivamente o problema de travar) -->
    <aside class="sidebar" id="sidebar">
        <div class="logo">
            <span class="avatar-mc">MC</span>
            <h2>MIRA confeitaria</h2>
        </div>
        
        <nav class="menu">
            <a href="home.php">
                <svg xmlns="http://w3.org" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="7" height="9" rx="1"/><rect x="14" y="3" width="7" height="5" rx="1"/><rect x="14" y="12" width="7" height="9" rx="1"/><rect x="3" y="16" width="7" height="5" rx="1"/></svg>
                <span>Dashboard</span>
            </a>
            <a href="clientes.php">
                <svg xmlns="http://w3.org" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
                <span>Clientes</span>
            </a>
            <a href="#" class="active">
                <svg xmlns="http://w3.org" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/></svg>
                <span>Produtos</span>
            </a>
            <a href="pedidos.php">
                <svg xmlns="http://w3.org" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="9 11 12 14 22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg>
                <span>Pedidos</span>
            </a>
            <a href="vendas.php">
                <svg xmlns="http://w3.org" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/></svg>
                <span>Vendas</span>
            </a>
            <a href="receitas.php">
                <svg xmlns="http://w3.org" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>
                <span>Receitas</span>
            </a>
            <a href="fornecedores.php">
                <svg xmlns="http://w3.org" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="1" y="3" width="15" height="13" rx="2" ry="2"/><polygon points="16 8 20 8 23 11 23 16 16 16 16 8"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>
                <span>Fornecedores</span>
            </a>
            <a href="financeiro.php">
                <svg xmlns="http://w3.org" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>
                <span>Controle Financeiro</span>
            </a>
            <a href="configuracoes.php">
                <svg xmlns="http://w3.org" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg>
                <span>Configurações</span>
            </a>
            <a href="../login.html">
                <svg xmlns="http://w3.org" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>
                <span>Sair</span>
            </a>
        </nav>

        <div class="sidebar-footer">
            <p class="brand-phrase">Doces feitos para transformar<br>momentos em memórias.</p>
            <div class="user-profile">
                <span class="avatar-lc">LC</span>
                <div class="user-info">
                    <h4>Lucas</h4>
                    <small>Proprietário</small>
                </div>
            </div>
        </div>
    </aside>

    <main class="main-content">
        <header class="topbar">
            <div class="topbar-left">
                <h1>Cardápio de Produtos</h1>
                <p class="subtitle">Gerencie os bolos, doces e especificações do seu menu em tempo real.</p>
            </div>
        </header>

        <div class="products-grid-layout-unified">
            <!-- COLUNA ESQUERDA: Central de Cadastro e Edição (60%) -->
            <div class="form-card">
                <div style="margin-bottom: 12px;">
                    <h3 class="card-section-title">Dados do Produto</h3>
                    <p style="font-size: 11px; color: var(--text-muted);">Preencha ou modifique as especificações do doce selecionado.</p>
                </div>
                
                <form id="productForm" class="custom-form" novalidate>
                    <div class="form-row full-width">
                        <label>Nome do Produto:</label>
                        <input type="text" id="prod_nome" placeholder="Ex: Bolo Brigadeiro Belga, Macaron de Pistache...">
                    </div>

                    <div class="form-row-grid">
                        <div class="form-row">
                            <label>Categoria:</label>
                            <select id="prod_categoria" class="premium-select">
                                <option value="" disabled selected>Selecione a categoria</option>
                                <option value="bolos">Bolos Artesanais</option>
                                <option value="doces">Doces Gourmet</option>
                                <option value="tortas">Tortas</option>
                                <option value="caixas">Caixas Presenteáveis</option>
                            </select>
                        </div>
                        <div class="form-row">
                            <label>Tamanho / Peso Base:</label>
                            <input type="text" id="prod_tamanho" placeholder="Ex: 1kg, 2kg, Aro 15, Aro 20...">
                        </div>
                    </div>

                    <div class="form-row-grid">
                        <div class="form-row">
                            <label>Preço de Venda (R$):</label>
                            <input type="text" id="prod_preco" placeholder="0,00">
                        </div>
                        <div class="form-row">
                            <label>Visibilidade no Cardápio:</label>
                            <select id="prod_ativo" class="premium-select">
                                <option value="sim" selected>Ativo (Visível no PDV)</option>
                                <option value="nao">Inativo (Oculto)</option>
                            </select>
                        </div>
                    </div>

                    <div class="form-row full-width">
                        <label>Sabores / Recheios Disponíveis:</label>
                        <textarea id="prod_sabores" placeholder="Ex: Creme de Ninho, Geléia de Morango, Ganache 50%..."></textarea>
                    </div>

                    <div class="form-row full-width">
                        <label>Descrição do Produto / Detalhes de Apresentação:</label>
                        <textarea id="prod_descricao" placeholder="Descreva os acabamentos, tipo de embalagem ou alertas para intolerantes..."></textarea>
                    </div>

                    <div class="form-actions-4">
                        <button type="button" class="btn">Salvar</button>
                        <button type="button" class="btn">Editar</button>
                        <button type="button" class="btn">Limpar</button>
                        <button type="button" class="btn">Excluir</button>
                    </div>
                </form>
            </div>

            <!-- COLUNA DIREITA: Tabela Dinâmica Conectada ao PDO do MySQL (40%) -->
            <div class="list-card">
                <div style="margin-bottom: 12px;">
                    <h3 class="card-section-title">Produtos Cadastrados</h3>
                    <p style="font-size: 11px; color: var(--text-muted);">Clique em uma linha para carregar ou digite para filtrar na hora.</p>
                </div>

                <div class="list-header" style="display: block; margin-bottom: 20px;">
                    <div class="search-box-client" style="width: 100%;">
                        <input type="text" id="searchProductInput" placeholder="Pesquisar produto por nome...">
                        <svg xmlns="http://w3.org" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
                    </div>
                </div>

                <div class="table-wrapper">
                    <table class="products-table" id="productsTable">
                        <thead>
                            <tr>
                                <th style="width: 70px;">ID</th>
                                <th>Nome do Produto</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php
                            try {
                                $sql = "SELECT * FROM produtos ORDER BY id DESC";
                                $stmt = $pdo->query($sql);
                                $produtos = $stmt->fetchAll(PDO::FETCH_ASSOC);

                                if (count($produtos) > 0) {
                                    foreach ($produtos as $linha) {
                                        echo "<tr class='prod-row' 
                                                  data-id='".htmlspecialchars($linha['id'])."' 
                                                  data-nome='".htmlspecialchars($linha['nome'])."' 
                                                  data-categoria='".htmlspecialchars($linha['categoria'])."' 
                                                  data-tamanho='".htmlspecialchars($linha['tamanho'])."' 
                                                  data-preco='".number_format($linha['preco'], 2, ',', '.')."' 
                                                  data-ativo='".htmlspecialchars($linha['ativo'])."' 
                                                  data-sabores='".htmlspecialchars($linha['sabores'])."' 
                                                  data-descricao='".htmlspecialchars($linha['descricao'])."'>";
                                        echo "<td>#" . str_pad($linha['id'], 3, '0', STR_PAD_LEFT) . "</td>";
                                        echo "<td class='item-name-cell'>" . htmlspecialchars($linha['nome']) . "</td>";
                                        echo "</tr>";
                                    }
                                } else {
                                    echo '<tr><td colspan="2" style="text-align: center; color: var(--text-muted); padding: 40px 0; font-style: italic;">Nenhum produto cadastrado no banco local.</td></tr>';
                                }
                            } catch (PDOException $e) {
                                echo '<tr><td colspan="2" style="text-align: center; color: #C94A4A; padding: 40px 0; font-weight: 600;">Erro ao ler tabela: ' . htmlspecialchars($e->getMessage()) . '</td></tr>';
                            }
                            ?>
                        </tbody>
                    </table>
                </div>
            </div>

        </div>
    </main>

    <script src="../js/produtos.js"></script>
</body>
</html>
